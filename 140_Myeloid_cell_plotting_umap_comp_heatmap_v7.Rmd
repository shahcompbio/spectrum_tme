...
seu_obj_sub_mp <- read_rds(paste0("/work/shah/uhlitzf/data/SPECTRUM/freeze/v6/Macrophages_processed.rds"))

seu_obj_sub_dc <- read_rds(paste0("/work/shah/uhlitzf/data/SPECTRUM/freeze/v6/DCs_processed.rds"))
...
comp_tbl_sample_sort_mp <- plot_data_sub %>%
  mutate(cell_type_super_ml = ifelse(cluster_label %in% grep("DC", names(clrs$cluster_label[[coi]]), value = T), "Dendritic cell", "Macrophage")) %>% 
  filter(therapy == "pre-Rx", consensus_signature != "Undetermined", cell_type_super_ml == "Macrophage", str_detect(cluster_label, "M[1-2]\\.")) %>%
  group_by(sample, tumor_subsite, tumor_supersite, tumor_megasite, patient_id_short,
           therapy, sort_short_x, consensus_signature, cluster_label, cell_type_super_ml) %>%
  tally %>%
  group_by(sample, tumor_subsite, tumor_supersite, tumor_megasite, patient_id_short,
...
plist3 <- default_comp_grid_list(
  comp_tbl_sample_sort_mp, cluster_label, "M1.CXCL10",
  cluster_label, super_type = "Myeloid.super", vec_plot = F)
...
plist5_no_yaxis <- default_comp_grid_list(
  comp_tbl_sample_sort_mp, cluster_label, "M2.MARCO",
  cluster_label, super_type = "Myeloid.super", vec_plot = F, yaxis = F)
...
comp_grid5_no_yaxis <- plot_grid(plotlist = plist5_no_yaxis, ncol = 1, align = "v", rel_heights = c(0.2, 0.2, 0.21, 0.39))

comp_grid6 <- plot_grid(plotlist = plist6, ncol = 1, align = "v", rel_heights = c(0.13, 0.13, 0.13, 0.17, 0.44))
...
comp_grid3_5 <- plot_grid(comp_grid3, comp_grid5_no_yaxis, ncol = 2, rel_widths = c(0.64, 0.36))
comp_grid3_5

ggsave("_fig/005_Myeloid_cell/005_Macrophage.cell_comp_3_5.pdf", comp_grid3_5, width = 4.5, height = 6)
...
comp_grid6
ggsave("_fig/005_Myeloid_cell/005_Myeloid.cell_comp.pdf", comp_grid6, width = 3, height = 10)

comp_grid7
ggsave("_fig/005_Myeloid_cell/005_Myeloid.cell_comp_no_ascites.pdf", comp_grid7, width = 3, height = 10)
