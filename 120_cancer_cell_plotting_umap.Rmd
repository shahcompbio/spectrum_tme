...
```{r chunk_063, fig.width=4, fig.height=5}
...
pcomp_grid_p_full <- plot_grid(plotlist = plist,
                               ncol = 1, align = "v",
                               rel_heights = c(0.2, 0.2, 0.24, 0.36))
...
```{r chunk_100, fig.width=14, fig.height=5}
...
pw_boxplot_mutsig <- filter(plot_data, pathway == "JAK.STAT") %>% 
  ggplot() +
  geom_boxplot(aes(consensus_signature, score, color = consensus_signature)) +
  scale_color_manual(values = clrs$consensus_signature) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  labs(x = "", y = "JAK/STAT pathway\nPROGENy score")

plot_box_grid <- plot_grid(pw_boxplot_mutsig + guides(color = F), 
                           pw_boxplot + guides(color = F), 
                           nrow = 1, align = "h", 
                           rel_widths = c(c(0.23, 0.77)))

pw_grid_full <- ggdraw() +
  draw_plot(pw_grid_left, x = 0, y = 0, width = 0.33, height = 1) +
  draw_plot(plot_box_grid, x = 0.35, y = 0.1, width = 0.65, height = 0.75)
