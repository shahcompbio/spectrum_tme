...
meta_tbl <- read_excel("_data/small/MSK SPECTRUM - Single cell RNA-seq.xlsx", sheet = 3) %>% 
...
write_tsv(marker_sheet, paste0("/work/shah/uhlitzf/data/SPECTRUM/freeze/v5/", coi, "_marker_sheet.tsv"))
...
# write_rds(seu_obj_sub, paste0("/work/shah/uhlitzf/data/SPECTRUM/freeze/v5/", coi, "_processed_filtered.rds"))
seu_obj_sub <- read_rds(paste0("/work/shah/uhlitzf/data/SPECTRUM/freeze/v5/", coi, "_processed_filtered.rds"))
seu_obj_sub$cluster_label <- seu_obj$cluster_label[colnames(seu_obj) %in% colnames(seu_obj_sub)]
write_rds(seu_obj_sub, paste0("/work/shah/uhlitzf/data/SPECTRUM/freeze/v5/", coi, "_processed_filtered.rds"))
...
write_tsv(select(plot_data_sub, cell_id, everything(), -UMAP_1, -UMAP_2, -contains("RNA_")), paste0("/work/shah/uhlitzf/data/SPECTRUM/freeze/v5/", coi, "_embedding.tsv"))

```

## sub cluster CD8 cells

```{r chunk_61, fig.width=12, fig.height=4}

# seu_obj_cd8 <- seu_obj_sub %>% 
#   subset(subset = cluster_label == "CD8.T") %>% 
#   FindNeighbors(reduction = "harmony", dims = 1:50) %>% 
#   FindClusters(seu_obj_cd8, resolution = 0.2) %>% 
#   RunUMAP(dims = 1:50, reduction = "harmony", reduction.name = "umapharmony", reduction.key = "umapharmony_")
# 
# write_rds(seu_obj_cd8, "/work/shah/uhlitzf/data/SPECTRUM/freeze/v5/CD8.T_processed.rds")
seu_obj_cd8 <- read_rds("/work/shah/uhlitzf/data/SPECTRUM/freeze/v5/CD8.T_processed.rds")

# marker_tbl <- FindAllMarkers(seu_obj_cd8, only.pos = T)
# write_tsv(as_tibble(marker_tbl), "/work/shah/uhlitzf/data/SPECTRUM/freeze/v5/CD8.T_markers.tsv")
marker_tbl <- read_tsv("/work/shah/uhlitzf/data/SPECTRUM/freeze/v5/CD8.T_markers.tsv")

# as_tibble(marker_tbl) %>% select(cluster, gene) %>% group_by(cluster) %>% 
...
ref_set <- markers_v5_super[["CD8.T"]] %>% 
  bind_rows(markers_doub_tbl) %>% 
  group_by(subtype) %>% 
  mutate(m = length(gene),
         n = length(rownames(seu_obj))-m,
...
formattable::formattable(marker_sheet)
write_tsv(marker_sheet, paste0("/work/shah/uhlitzf/data/SPECTRUM/freeze/v5/CD8.T.cell_marker_sheet.tsv"))


```

```{r chunk_62, fig.width=12, fig.height=4}
...
# write_rds(seu_obj_sub_sub, "/work/shah/uhlitzf/data/SPECTRUM/freeze/v5/T.cell_processed_filtered_sub.rds")

```

```{r chunk_63, fig.width=12, fig.height=4}
...
# write_rds(seu_obj_sub_sub, "/work/shah/uhlitzf/data/SPECTRUM/freeze/v5/T.cell_processed_filtered_sub.rds")
# seu_obj_sub_sub <- read_rds("/work/shah/uhlitzf/data/SPECTRUM/freeze/v5/T.cell_processed_filtered_sub.rds")

signature_modules <- read_excel("_data/small/signatures/SPECTRUM v5 sub cluster markers.xlsx", 
...
# 
# write_rds(seu_obj_cd8_sub, "/work/shah/uhlitzf/data/SPECTRUM/freeze/v5/CD8.T_processed_filtered.rds")
# 
# dpt_obj <- DPT(dc_obj)
# write_rds(dpt_obj, "/work/shah/uhlitzf/data/SPECTRUM/freeze/v5/CD8.T_sub_dpt.rds")
...
```{r chunk_69, fig.width=12, fig.height=4}

# set.seed(1)
# seu_foo <- subset(seu_obj_cd8_sub, cells = c(root_cell, sample(colnames(seu_obj_cd8_sub), 1000)))
# 
# dc_obj_foo <- DiffusionMap(seu_foo@reductions$harmony@cell.embeddings, k = 100)
# dc_mat_foo <- dc_obj_foo@eigenvectors
# colnames(dc_mat_foo) <- paste0("DC_", 1:ncol(dc_mat_foo))
# seu_foo[["DC"]] <- CreateDimReducObject(embeddings = dc_mat_foo, key = "DC_", assay = DefaultAssay(seu_foo))
# 
# dpt_obj_foo <- DPT(dc_obj_foo)
# seu_foo$DPT1 <- dpt_obj_foo$DPT1
# 
# head(dpt_obj_foo$DPT1)
# 
# foo_tbl <- as_tibble(cbind(cell_id=colnames(seu_obj_cd8_sub),FetchData(seu_obj_cd8_sub, c("DC_1", "DC_2", "cluster_label", "DPT1"))))
# p1 <- ggplot(foo_tbl, aes(DC_1, DC_2, color = DPT1)) + 
#   geom_point(size = 0.01, alpha = 0.1) +
#   geom_point(color = "red", data = filter(foo_tbl, cell_id == root_cell)) + 
#   theme_void() +
#   scale_color_gradientn(colors = magma(9))
# p2 <- ggplot(foo_tbl, aes(DC_1, DC_2, color = cluster_label)) + 
#   geom_point(size = 0.01, alpha = 0.1) + 
#   theme_void()
# plot_grid(p1, p2)
# 
...
```{r chunk_70, fig.width=12, fig.height=4}
...
comp_tbl_sample_sort <- plot_data_sub %>% 
  ...
  mutate(tumor_subsite = ordered(tumor_subsite, levels = unique(tumor_subsite)),
         ...
         patient_id = ordered(patient_id, levels = signature_tbl$patient_id)) %>% 
  arrange(patient_id) %>% 
...
comp_tbl_sample_sort %>% 
  ...
  write_tsv(paste0("/work/shah/uhlitzf/data/SPECTRUM/freeze/v5/", coi, "_subtype_compositions.tsv"))
...
pcomp_hist_grid
ggsave(paste0("_fig/_paper/02_composition_v5_",coi,".pdf"), pcomp_hist_grid, width = 10, height = 2)
...
### per mutational signature

Subset of pre-Rx adnexa samples

```{r chunk_150, fig.width=8, fig.height=5}

ggplot(filter(comp_tbl_sample_sort, therapy == "pre-Rx", tumor_supersite == "Adnexa")) +
  geom_boxplot(aes(consensus_signature, nrel, fill = consensus_signature)) +
  geom_point(aes(consensus_signature, nrel)) +
  facet_wrap(~cluster_label) +
  scale_fill_manual(values = clrs$consensus_signature) +
