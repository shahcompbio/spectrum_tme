...
## slim cohort data
cells_tbl <- read_tsv("/work/shah/uhlitzf/data/SPECTRUM/freeze/cohort_slim_normalized_v4.tsv") %>% 
  left_join(consOV_tbl, by = "cell_id") %>% 
  left_join(select(seu_meta, sample = isabl_id, patient_id, contains("tumor"), 
...
umap_consensusOV <- base_umap + 
  geom_point(aes(UMAP_1, UMAP_2, color = consensusOV_label), 
             size = pt.size, alpha = pt.alpha) +
  scale_color_manual(values = consensusOV_colors) +
...
umap_consensusOV_facets <- base_umap + 
  geom_point(aes(UMAP_1, UMAP_2), color = "grey90", 
             size = pt.size, alpha = pt.alpha,
             data = select(seu_tbl_full, -patient_id_short)) +
  geom_point(aes(UMAP_1, UMAP_2, color = consensusOV_label), 
             size = pt.size, alpha = pt.alpha) +
  scale_color_manual(values = consensusOV_colors) +
  guides(color = F) +
...
```{r chunk_060, fig.width=12, fig.height=8}

celltype_order <- c("Monocyte", "Plasma cell", "T reg cell", "Cytotoxic T cell", "NK cell", "CD4 T cell", "B cell", "pDC", "Ovarian cancer cell", "Endothelial cell", "Mesothelial cell", "Myofibroblast", "Vascular SMC")
 
consOV_tbl_summary <- seu_tbl_full %>%
  select(cell_type, consensusOV_label, patient_id_short) %>%
  group_by(cell_type, consensusOV_label, patient_id_short) %>%
  tally %>%
  complete(cell_type, consensusOV_label, fill = list(n = 0)) %>%
  group_by(cell_type, patient_id_short) %>%
  mutate(nrel = n/(sum(n))*100) %>%
  mutate(consensusOV_label = ordered(consensusOV_label, levels = c(names(consensusOV_colors)))) %>%
  arrange(consensusOV_label, nrel) %>%
  ungroup() #%>%
  #mutate(cell_type = ordered(str_replace_all(cell_type, "\\.", " "), levels = celltype_order))

ggplot(consOV_tbl_summary) +
  geom_bar(aes(cell_type, nrel, fill = consensusOV_label),
           stat = "identity") +
  scale_x_discrete(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0)) +
  scale_fill_manual(values = consensusOV_colors) +
...
ggplot(consOV_long) + 
  geom_boxplot(aes(key, value, color = key)) + 
  scale_color_manual(values = consensusOV_colors) +
  facet_wrap(~cell_type, ncol = 9) + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) + 
  labs(y = "RF posterior probability", x = "")

```

```{r chunk_090, fig.width=6, fig.height=3}
...
plot_rf <- function(subtype, aty = element_blank()) {
  consOV_long %>% 
    ...
    scale_color_manual(values = consensusOV_colors, guide = F) +
...
```{r chunk_100, fig.width=6, fig.height=2}
...
plot_subtype_heat <- function(subtype) {
  consOV_median %>% 
    ...
    geom_tile(aes(patient_id_short, cell_type, alpha = value_scaled), 
              fill = consensusOV_colors[subtype]) +
...
```{r chunk_110, fig.width=6, fig.height=2.5}
