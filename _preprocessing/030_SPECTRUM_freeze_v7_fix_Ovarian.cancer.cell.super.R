...
meta_tbl <- read_excel("_data/small/MSK SPECTRUM - Single cell RNA-seq_v7.xlsx", sheet = 2) %>% 
...
preprocess_wrapper <- . %>%
  ...
  RunUMAP(reduction = "pca", dims = 1:50, reduction.name = "umappca", reduction.key = "umappca_") %>%
  RunUMAP(reduction = "harmony", dims = 1:50, reduction.name = "umapharmony", reduction.key = "umapharmony_")
...
write_rds(seu_obj_sample, paste0("/work/shah/uhlitzf/data/SPECTRUM/freeze/v7/", coi, "_highqc_sample.rds"))
seu_obj_sample <- read_rds(paste0("/work/shah/uhlitzf/data/SPECTRUM/freeze/v7/", coi, "_highqc_sample.rds"))
...
write_rds(seu_obj_sub, paste0("/work/shah/uhlitzf/data/SPECTRUM/freeze/v7/", coi, "_highqc.rds"))
seu_obj_sub <- read_rds(paste0("/work/shah/uhlitzf/data/SPECTRUM/freeze/v7/", coi, "_highqc.rds"))

Idents(seu_obj_sub) <- seu_obj_sub$RNA_snn_res.0.2
marker_tbl_02 <- as_tibble(FindAllMarkers(seu_obj_sub, only.pos = T))
write_tsv(marker_tbl_02, paste0("/work/shah/uhlitzf/data/SPECTRUM/freeze/v7/", coi, "_highqc_markers_02.tsv"))
marker_tbl_02 <- read_tsv(paste0("/work/shah/uhlitzf/data/SPECTRUM/freeze/v7/", coi, "_highqc_markers_02.tsv"))

Idents(seu_obj_sub) <- seu_obj_sub$RNA_snn_res.0.3
marker_tbl_03 <- as_tibble(FindAllMarkers(seu_obj_sub, only.pos = T))
write_tsv(marker_tbl_03, paste0("/work/shah/uhlitzf/data/SPECTRUM/freeze/v7/", coi, "_highqc_markers_03.tsv"))
marker_tbl_03 <- read_tsv(paste0("/work/shah/uhlitzf/data/SPECTRUM/freeze/v7/", coi, "_highqc_markers_03.tsv"))
