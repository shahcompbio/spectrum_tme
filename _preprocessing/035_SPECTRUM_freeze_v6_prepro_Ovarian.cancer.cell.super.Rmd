...
myfeatures <- c("UMAP_1", "UMAP_2", "umapharmony_1", "umapharmony_2", "sample", "RNA_snn_res.0.1", "doublet", "nCount_RNA", "nFeature_RNA", "percent.mt", "doublet_score")
...
enframe(sort(table(seu_obj$cluster_label))) %>% 
  ...
  ggplot() +
  geom_bar(aes(name, log10(value)), stat = "identity") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  labs(y = c("log10 #cells"), x = "cluster") +
  background_grid(color.major = "black")
...
```{r chunk_51, fig.width=15, fig.height=12}

ggplot(plot_data, aes(umapharmony_1, umapharmony_2, color = cluster_label)) + 
  geom_point(data = select(plot_data, -cluster_label), color = "grey80", size = 0.01) +
  common_layers_disc +
...
# seu_obj_sub <- RunUMAP(seu_obj_sub, dims = 1:50, reduction = "pca", reduction.name = "umappca", reduction.key = "umappca_")
...
plot_data_sub <- as_tibble(FetchData(seu_obj_sub, c(myfeatures, "cluster_label", "umappca_1", "umappca_2"))) %>% 
  left_join(select(meta_tbl, sample = isabl_id, patient_id, tumor_supersite, tumor_subsite, sort_parameters, therapy), 
            by = "sample") %>% 
  mutate(patient_id = str_remove_all(patient_id, "SPECTRUM-OV-"),
         tumor_supersite = ordered(tumor_supersite, levels = names(clrs$tumor_supersite))) %>% 
...
ggplot(plot_data_sub, aes(umappca_1, umappca_2, color = cluster_label)) + 
  common_layers_disc +
  ggtitle("Sub cluster") +
  #facet_wrap(~cluster_label) +
  scale_color_manual(values = clrs$cluster_label[[coi]])

ggplot(plot_data_sub, aes(umappca_1, umappca_2, color = patient_id)) + 
  common_layers_disc +
  ggtitle("Patient") +
  # facet_wrap(~therapy) +
  scale_color_manual(values = clrs$patient_id)

ggplot(plot_data_sub, aes(umappca_1, umappca_2, color = tumor_supersite)) + 
  common_layers_disc +
  ggtitle("Site") +
  scale_color_manual(values = clrs$tumor_supersite)
...
```{r chunk_80, fig.width=12, fig.height=4}

# FeaturePlot(seu_obj_sub, c("ACTA2", "FAP", "MMP11", "COL11A1", "COL12A1", "COL14A2"), reduction = "umapharmony", combine = F) %>% lapply(add, NoAxes()) %>% lapply(add, NoLegend()) %>% CombinePlots()
