...
#   arrange(DC_1) %>%
#   do(fit = tibble(
#     smooth = broom::tidy(predict(loess(value ~ DPT1, data = .)))$x,
#     DPT1 = .$DPT1)) %>%
#   unnest(fit) %>%
#   group_by(key) %>%
#   mutate(scaled_smooth = scales::rescale(smooth))
# 
...
module_curves <- ggplot(plot_data_smooth) +
  geom_line(aes(DPT1, scaled_smooth, color = key), size = 1.5)+
...
mutsig_boxplot_jakstat <- filter(plot_data, key == "JAK.STAT.pathway") %>% 
...
ggdraw() +
  ...
  draw_plot(module_curves, x = 0.36, y = 0, width = 0.25, height = 1) +
...
cor_plot1 <- plot_data_cor %>% 
  ...
  ggplot() +
  geom_point(aes(mean_value_tc, mean_value_cc, color = consensus_signature)) +
  geom_smooth(aes(mean_value_tc, mean_value_cc), method = "lm", se = T,
              linetype = 2, size = 0.5, color = "black") +
  stat_cor(aes(mean_value_tc, mean_value_cc), 
           method = "spearman", color = "black") +
  scale_color_manual(values = clrs$consensus_signature) +
  labs(x = "Dysfunctional score\n(CD8+ T cells)", 
       y = "CD274 expression\n(cancer cells)",
       color = "Mutational\nsignature")

cor_plot2 <- {cor_plot1 + 
    labs(x = "JAK-STAT score\n(CD8+ T cells)", 
         y = "JAK-STAT score\n(cancer cells)",
         color = "Mutational\nsignature")
} %+%
...
plist1 <- default_comp_grid_list(
  comp_tbl_cd8, cluster_label_sub, "CD8.T.naive.1", nmax = 2000,
  cluster_label_sub, super_type_sub = "T.super", vec_plot = F)

plist2 <- default_comp_grid_list(
  comp_tbl_cd8, cluster_label_sub, "CD8.T.naive.2", nmax = 2000, 
  cluster_label_sub, super_type_sub = "T.super", vec_plot = F)
...
plist4 <- default_comp_grid_list(
  comp_tbl_cd8, cluster_label_sub, "CD8.T.CXCL13.early", nmax = 2000, 
  cluster_label_sub, super_type_sub = "T.super", vec_plot = F)

plist5 <- default_comp_grid_list(
  comp_tbl_cd8, cluster_label_sub, "CD8.T.CXCL13.late", nmax = 2000, 
  cluster_label_sub, super_type_sub = "T.super", vec_plot = F)
